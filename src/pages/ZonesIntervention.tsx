...
import { MapPin, ChevronRight, ArrowRight, Phone, CheckCircle2, Star } from "lucide-react";
import { Button } from "@/components/ui/button";
import Navbar from "@/components/Navbar";
import Footer from "@/components/Footer";
import { idfCities } from "@/data/idfCities";
    ...
    <main>
      ...
      <section className="py-16">
        <div className="container mx-auto px-4">
          ...
          {/* Section Île-de-France avec tous les liens */}
          <div className="mb-12 p-8 bg-accent/5 rounded-2xl border border-accent/20">
            <h2 className="font-display text-2xl font-bold text-foreground mb-6">Toutes nos zones d'intervention en Île-de-France</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              {Object.values(idfCities).flat().map((city) => (
                <Link
                  key={city.slug}
                  to={`/zones-intervention/reparation-volet-${city.slug}`}
                  className="p-3 rounded-lg text-sm font-medium bg-background border border-border hover:border-accent hover:bg-accent/5 transition-all text-foreground hover:text-accent"
                >
                  {city.name}
                </Link>
              ))}
              ...
              >
                <div className="flex flex-col lg:flex-row lg:items-start gap-6">
                  <div className="flex-1">
                    ...
                    {/* Villes Île-de-France avec liens */}
                    {r.slug === "ile-de-france" && (
                      <div className="mb-4 p-4 bg-accent/5 rounded-lg border border-accent/20">
                        <h4 className="text-xs font-bold text-foreground uppercase tracking-wider mb-3">Nos villes en Île-de-France</h4>
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                          {Object.values(idfCities).flat().map((city) => (
                            <Link
                              key={city.slug}
                              to={`/zones-intervention/reparation-volet-${city.slug}`}
                              className="px-3 py-2 rounded text-xs font-medium bg-accent/10 text-accent hover:bg-accent/20 border border-accent/30 transition-all hover:border-accent/50"
                            >
                              {city.name}
                            </Link>
                          ))}
                    ...
                    {/* Lien "Découvrir toutes nos zones" pour Île-de-France */}
                    {r.slug === "ile-de-france" && (
                      <div className="mt-4">
                        <Link
                        ...
                        >
                          Voir toutes les zones d'intervention <ArrowRight className="h-4 w-4" />
                        </Link>
                      </div>
                    )}
                  </div>
